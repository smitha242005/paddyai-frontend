...
async function analyzeImage() {
  ...
  try {
    ...
    const parsed = {
      cropType: 'Rice (Paddy)',
      healthStatus: d.disease === 'Healthy' ? 'Healthy' : 'Damaged',
      damageType: d.disease === 'Healthy' ? null : d.disease,
      soilType: 'Clayey Loam',
      soilPH: '6.0 – 7.0',
      fertilizer: 'NPK 120:60:60 kg/ha',
      pesticide: d.pesticide || 'No pesticide required',
      waterPerDay: '5–6 litres/day',
      irrigationMethod: 'Flood irrigation (2–5 cm standing water)',
      growthMonth: 'June – October',
      harvestMonth: 'October – November',
      estimatedYield: '4.5 – 5.5 t/ha',
      treatmentPlan: d.recovery || 'No treatment required',
      medicine: d.medicine || 'No medicine required',
      generalTips:
        d.disease !== 'Healthy'
          ? `Severity: ${d.severity}. Apply ${d.medicine}. Use ${d.pesticide}. ${d.recovery}`
          : 'Crop appears healthy. Continue normal irrigation and fertilizer schedule.'
    };
    ...
    document.getElementById('results-area').innerHTML = `
      <div style="
        ...
        color:#b91c1c;
        line-height:1.7">
        ⚠️ Failed to connect to backend.<br><br>
...
function renderResults(d) {
  ...
  const healthCls = d.healthStatus === 'Healthy'
    ? 'healthy'
    : d.healthStatus === 'Damaged'
    ? 'damaged'
    : 'partial';
    ...
    <div class="result-grid">
      <div class="r-card">
        ...
        <span class="health-badge ${healthCls}">
          ${healthEmoji} ${d.healthStatus}
        </span>
        ${
          d.damageType
            ? `<div class="r-sub" style="margin-top:.5rem;color:#b91c1c">
                Disease: ${d.damageType}
              </div>`
            : ''
        }
      </div>
        ...
        ${
          d.medicine
            ? `<div style="
                ...
                margin-bottom:1rem;
                font-size:.88rem">
                <strong style="color:#b91c1c">💊 Medicine:</strong>
                ${d.medicine}
              </div>`
            : ''
        }
  ...
  html += `
    ...
    <button class="btn-new" onclick="clearImage()">
      ...
      <tr>
        <td colspan="7">
          <div class="empty-history">
            <span>🌱</span>
            No analyses yet. Upload a paddy leaf image to begin.
          </div>
        </td>
      </tr>
    `;
