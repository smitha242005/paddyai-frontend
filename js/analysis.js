...
async function analyzeImage() {
  ...
  try {
    ...
    const yieldData = await yieldResp.json();
...
function renderResults(d) {
  ...
  let html = `
    <div class="result-grid">
      ...
      <div class="r-card blue"><div class="r-label">🧪 Soil Recommendation</div><div class="r-val">${d.soilType}</div><div class="r-sub">Optimal pH: ${d.soilPH}</div></div>
      <div class="r-card"><div class="r-label">💧 Water Requirement</div><div class="r-val">${d.waterPerDay}</div><div class="r-sub">${d.irrigationMethod}</div></div>
      <div class="r-card gold"><div class="r-label">📅 Growth Period</div><div class="r-val">${d.growthMonth}</div><div class="r-sub">Harvest: <strong>${d.harvestMonth}</strong></div></div>
      <div class="r-card blue"><div class="r-label">🌿 Fertilizer</div><div class="r-val" style="font-size:.95rem">${d.fertilizer}</div></div>
      <div class="r-card purple"><div class="r-label">📦 Estimated Yield</div><div class="r-val">${d.estimatedYield}</div></div>
      ...
      ${d.damageType ? `<div class="r-card danger"><div class="r-label">🦠 Disease Detected</div><div class="r-val">${d.damageType}</div></div>` : ''}
    </div>`;
  if (isDamaged && d.treatmentPlan) {
    html += `<div class="full-card" style="border-left:4px solid #ef4444">
  ...
  html += `<div class="full-card"><h4>💡 Cultivation Tips</h4><div class="prose">${d.generalTips}</div></div>
